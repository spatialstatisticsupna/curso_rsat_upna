...
Data portals are straightforward and intuitive. However, they can be tedious when requesting images for longer periods of time or several data products. In these situations, `rsat` becomes handy. The package enables you to send requests from `R` to the appropriate web service easily and programatically using *Application Programming Interfaces (APIs)*. You will be able to judge yourself at the end of this chapter where the same example is repeated using `rsat`.

### Data products

#### Names

As mentioned earlier, these tutorial mostly concern ***optical multispectral images*** capturing ***surface reflectance*** values (`rsat` gives access to products with other levels of pre-processing). Web services name their products using short-names or ***ID**s*. The product names for the on-going missions are listed below. We rely on `rsat`'s manual and tables like the one below to remember the *IDs* of each data product. After a while you will learn them by heart.
...
-   ***Spatial resolution*** is related to the area covered by a single pixel. High-resolution images contain small pixels, i.e. they cover small surface areas. Among the missions mentioned before, Sentinel-2 is the one offering images with the highest resolution ( $\sim 10\times10m$ per pixels).
...
-   The **spectral resolution** corresponds to the number of bands and their wavelength range. The MODIS sensor, with its $36$ bands in total, is the sensor with the highest spectral resolution.
    ...
    ![Spectral bands for several satellte missions (Landsat 7/8, MODIS, Sentinel-2)](imgs/bands_missions.png "Spectral bands for several satellte missions (Landsat 7/8, MODIS, Sentinel-2)"){width="485" height="223"}
...
Our paths are `db.path` and `rtoi.path` for saving the raw and the processed datasets
...
```{r echo=FALSE}
navarre <- read_rtoi("Z:/CursoRSAT/regions/Navarre")
```
...
Your turn: We want you to search ([***do not download!***]{.ul}) the multispectral images captured by Landsat-8 (`"LANDSAT_8_C1"`), Sentinel-2 (`"S2MSI"`) and MODIS (`"mod09ga"`) in Navarre during May $2019$. Try to do it with one single `sat_search()` instruction. Use [***your own credentials***]{.ul} to check your access to the different data portals.
